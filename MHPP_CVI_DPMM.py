...
from Auxiliary_functions_for_TFCVI import E_log_p_of_z_given_other_z, ELBO_term_for_pz,\
                                          lowerbound, Update_MHPP_parameters
                                            


def MHPP_DPMM(X, T=10, max_iter=100, alpha=1., thresh=1e-3, delta=.1):
        ...
        for iteration in range(max_iter):
            ...
            if batch > 0:
                ...
                for t in range(T):
                    ...
                    b_0[t] = E_q_rho[0, t]*static_b_tao[t] + (1-E_q_rho[0, t]) +\
                        .5*E_q_rho[0, t]*(static_cov_mu[t].dot(static_mean_mu[t])).dot(static_mean_mu[t]) -\
                        .5*(prior_cov[t].dot(prior_mean[t])).dot(prior_mean[t])
                a_0 = E_q_rho[0]*(static_a_tao - .5) + 1. -.5*E_q_rho[0]
           ...
           # Update tao
            ...
            if batch > 0:
                # Update HPP
                omega = Update_MHPP_parameters(a_tao, b_tao, static_a_tao, static_b_tao,\
